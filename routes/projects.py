...
def register_routes(app: FastAPI) -> None:
    ...
    async def create_project_chat_message(request: Request, project_id: str, payload: ProjectChatMessageCreateIn):
        ...
        # --- Auto-detect plan approval/rejection from owner chat messages ---
        try:
            is_owner = str(actor.get("type") or "").strip().lower() in ("user", "owner")
            if is_owner:
                raw_text = str(payload.text or "").strip().lower()
                _APPROVE_KEYWORDS = (
                    "setujui plan", "setuju plan", "approve plan", "approved",
                    "plan approved", "approve", "setuju", "acc plan", "ok plan",
                    "lanjut plan", "gas plan", "lgtm", "looks good",
                    "setujui", "approve the plan", "go ahead",
                )
                _REJECT_KEYWORDS = (
                    "reject plan", "tolak plan", "revisi plan", "ulangi plan",
                    "regenerate plan", "redo plan", "plan ulang",
                )
                _pc = db()
                try:
                    plan_row = _pc.execute(
                        "SELECT plan_status FROM projects WHERE id = ?", (project_id,),
                    ).fetchone()
                finally:
                    _pc.close()
                current_plan = _coerce_plan_status(plan_row["plan_status"] if plan_row else None)
                if current_plan == PLAN_STATUS_AWAITING_APPROVAL:
                    if any(kw in raw_text for kw in _APPROVE_KEYWORDS):
                        import asyncio
                        now = int(time.time())
                        approve_conn = db()
                        try:
                            approve_conn.execute(
                                "UPDATE projects SET plan_status = ?, plan_approved_at = ?, plan_updated_at = ? WHERE id = ?",
                                (PLAN_STATUS_APPROVED, now, now, project_id),
                            )
                            user_id = str(actor.get("id") or "").strip()
                            append_project_activity_log_entry(
                                approve_conn, project_id=project_id, actor_type="user",
                                actor_id=user_id, actor_label="owner",
                                event_type="project.plan.approved",
                                summary="Owner approved project plan via chat",
                                payload={"source": "chat_auto_detect"}, created_at=now,
                            )
                            approve_conn.commit()
                        finally:
                            approve_conn.close()
                        _refresh_project_documents(project_id)
                        await emit(project_id, "project.plan.approved", {"project_id": project_id, "status": PLAN_STATUS_APPROVED})
                    ...
                    elif any(kw in raw_text for kw in _REJECT_KEYWORDS):
                        now = int(time.time())
                        reject_conn = db()
                        try:
                            reject_conn.execute(
                                "UPDATE projects SET plan_status = ?, plan_updated_at = ? WHERE id = ?",
                                (PLAN_STATUS_PENDING, now, project_id),
                            )
                            reject_conn.commit()
                        finally:
                            reject_conn.close()
                        _refresh_project_documents(project_id)
                        await emit(project_id, "project.plan.rejected", {"project_id": project_id, "status": PLAN_STATUS_PENDING})
        except Exception as exc:
            print(f"[project_chat] plan approval auto-detect failed for project {project_id}: {exc}", flush=True)
    ...
    async def accept_project_external_agent_invite(request: Request, invite_token: str, payload: ProjectExternalAgentInviteAcceptIn):
        ...
        for item in normalized_agents:
            ...
            conn.execute(
                ...
                """,
                (project_id, agent_id, _hash_access_token(raw_project_agent_token), raw_project_agent_token, now),
            )
